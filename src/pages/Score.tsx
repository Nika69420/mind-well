...
function Score() {
  ...
  const score: ScoreData = useMemo(() => {
    if ((sessionStorage.getItem("MindCheckUserScore.v2") as string) !== null) {
      ...
      if (error instanceof FirebaseError) {
        switch (error.code) {
          case "auth/popup-closed-by-user": {
            toast.error("Do not close popup before selecting email.");
            break;
          }
          default: {
    ...
    <motion.main
      ...
      <div className="w-full max-w-7xl mx-auto py-12 px-8 flex flex-col mt-12">
        ...
        <h2 className="text-lg md:text-2xl font-medium text-center max-w-2xl mx-auto mt-4">
          Discover your mental well-being with a personalized score and helpful
          feedback.
        </h2>
        ...
        <section className="flex flex-col items-center text-center gap-6 max-w-2xl mx-auto mt-6">
          <div
            className={`${
              score.calculatedScore >= 0 && score.calculatedScore <= 33
                ? "border-green-500 text-green-500"
                : score.calculatedScore >= 34 && score.calculatedScore <= 66
                ? "border-orange-500 text-orange-500"
                : "border-red-500 text-red-500"
            } w-40 h-40 border-8 text-4xl flex items-center justify-center font-bold font-heading rounded-full`}>
            {score.calculatedScore} <span className="text-sm">/100</span>
          </div>
          <p className="text-xl font-semibold">{feedbackOnScore}</p>
          <p>
            <b>Note:</b> If you experience persistent scores above 10 or have
            any suicidal feelings, it's important to seek professional treatment
            and consult with a mental health professional immediately.
          </p>
          <p>
            Remember, the app is designed to provide guidance and support, but
          ...
          {user ? (
            ...
            <button
              ...
              className={`${
                loginLoading ? "text-gray-500 cursor-default" : ""
